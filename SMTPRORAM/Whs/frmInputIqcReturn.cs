...
using SMTPRORAM.Staging;

namespace SMTPRORAM.Whs
{
    public partial class frmInputIqcReturn : Form
    {
        private List<SYS_UserButtonFunction> lstUBFunction;
        public frmInputIqcReturn()
        {
            InitializeComponent();
        }

        private void frmInputIqcReturn_Load(object sender, EventArgs e)
        {
        ...
        private void iconbtnUpload_Click(object sender, EventArgs e)
        {
            if (!txtFile.Text.Trim().Equals(""))
            {
                ...
                for (rCnt = 2; rCnt <= range.Rows.Count; rCnt++)
                {
                    ...
                    {
                        bool _CheckExistID;
                        var rcInput = new WHS_INPUT_IQC_RETURN();
                        _CheckExistID = WHSInputIqcReturnDAL.Instance.CheckExistID(_ID);
                        if (!_CheckExistID)
                        {
                            ...
                            {
                                rcInput.NGAYTHUHOI = null;
                            }
                            else
                            {
                                rcInput.NGAYTHUHOI = DateTime.Parse(str[1].ToString().Trim());
                            }
                            rcInput.PARTCODE = str[2].ToString().Trim();
                            rcInput.SUPPILER = str[3].ToString().Trim();
                            rcInput.QTY = int.Parse(str[4].ToString().Trim());
                            rcInput.LYDOTHUHOI = str[5].ToString().Trim();
                            rcInput.NGUOITHUHOI = str[6].ToString().Trim();                           
                            ...
                            WHSInputIqcReturnDAL.Instance.Add(rcInput);
                        }
                        else
                        {
                            ...
                            rcInput.UpdatedDate = CommonDAL.Instance.getSqlServerDatetime();
                            WHSInputIqcReturnDAL.Instance.Update(rcInput);
                        }
        ...
        private void iconbtnPaste_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (o.GetDataPresent(DataFormats.Text))
                {
                    ...
                    dgView.Columns.Add("NGAYTHUHOI", "NGAYTHUHOI");
                    dgView.Columns.Add("PARTCODE", "PARTCODE");
                    dgView.Columns.Add("SUPPILER", "SUPPILER");
                    dgView.Columns.Add("QTY", "QTY");
                    dgView.Columns.Add("LYDOTHUHOI", "LYDOTHUHOI");
                    dgView.Columns.Add("NGUOITHUHOI", "NGUOITHUHOI");
                ...
                if (dgView.Rows.Count > 0)
                {
                    ...
                    foreach (DataGridViewRow row in dgView.Rows)
                    {
                        ...
                        if (!row.IsNewRow)
                        {
                            ...
                            if (row.Cells["NGAYTHUHOI"].Value == null)
                            {
                                rcInput.NGAYTHUHOI = null;
                            }
                            else
                            {
                                rcInput.NGAYTHUHOI = DateTime.Parse(row.Cells["NGAYTHUHOI"].Value.ToString());
                            }
                            if (row.Cells["SUPPILER"].Value == null)
                            {
                                rcInput.SUPPILER = "";
                            }
                            else
                            {
                                rcInput.SUPPILER = row.Cells["SUPPILER"].Value.ToString();
                            }
                            ...
                            {
                                rcInput.QTY = int.Parse(row.Cells["QTY"].Value.ToString());
                            }

                            if (string.IsNullOrEmpty(row.Cells["LYDOTHUHOI"].Value.ToString()))
                            {
                                rcInput.LYDOTHUHOI = "";
                            }
                            else
                            {
                                rcInput.LYDOTHUHOI = row.Cells["LYDOTHUHOI"].Value.ToString();
                            }

                            if (string.IsNullOrEmpty(row.Cells["NGUOITHUHOI"].Value.ToString()))
                            {
                                rcInput.NGUOITHUHOI = "";
                            }
                            else
                            {
                                rcInput.NGUOITHUHOI = row.Cells["NGUOITHUHOI"].Value.ToString();
                            }

                          

                            if (string.IsNullOrEmpty(row.Cells["PARTCODE"].Value.ToString()))
                            {
                                rcInput.PARTCODE = "";
                            }
                            else
                            ...
                            try
                            {
                                if (WHSInputIqcReturnDAL.Instance.CheckExistID(rcInput.ID) == true)
                                {
                                    rcInput.UpdatedDate = CommonDAL.Instance.getSqlServerDatetime();
                                    WHSInputIqcReturnDAL.Instance.Update(rcInput);
                                }
        ...
        private void iconbtnDelete_Click(object sender, EventArgs e)
        {
            if (dgView.Rows.Count > 0)
            {
                ...
                bool result = WHSInputIqcReturnDAL.Instance.Remove(ID);
                if (result)
                {
        ...
        private void saveEditDataToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (iconbtnEditmode.Enabled == true)
            {
                if (dgView.RowCount > 0)
                {
                    ...
                    {                       
                        rcInput.UpdatedDate = CommonDAL.Instance.getSqlServerDatetime();
                         result=WHSInputIqcReturnDAL.Instance.Update(rcInput);
                    }
                    catch (Exception)
                    {
