...
{
    public partial class frmEQRepairHistory : Form
    {
        ...
        private long Id = 0;
        public frmEQRepairHistory()
        {
            InitializeComponent();
        }

        private void frmEQRepairHistory_Load(object sender, EventArgs e)
        {
            _enable(false);
        ...
        private void iconbtnEdit_Click(object sender, EventArgs e)
        {
            ...
            {				
				MessageBox.Show("Phải chọn dữ liệu để sửa, chỉ sửa được dữ liệu bạn đã tạo ra?", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
        }
        private void ShowHistoryByUser()
        {
            CommonDAL.Instance.ShowDataGridViewWithDataTable(dgView, JIGREPAIRHISTORY_DAL.Instance.getHistoryByUser(Program.UserId));
        }
        private bool IsDataOK()
        {
			...
			{
				MessageBox.Show("Phải lựa chọn tình trạng cho thiết bị !!!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Error);
				cbStatus.Focus();
				return false;
			}	
			return true;
		...
		private void iconbtnSave_Click(object sender, EventArgs e)
        {
            if (IsDataOK())
            {                
                var repairHistory = new JIG_REPAIRHISTORY();
                ...
                try
                {
					...
					{
						string message = "";
						message = JIGREPAIRHISTORY_DAL.Instance.Add(repairHistory);
						if (message == "")
						{
							// Update tình trạng vào bảng thiết bị thông qua mã thiết bị.
							// Nếu thiết bị có tình trạng NG thì không được xuất nhập jig
							string mes = "";
							mes = JIGCALDEVICES_DAL.Instance.UpdateSStatus(repairHistory.ControlNo, repairHistory.RStatus, repairHistory.CreatedDate);
							if (mes!="")
							{
								MessageBox.Show("Không update được tình trạng thiết bị vào bảng quản lý thiết bị\n " +
                    ...
                    if (AddFlag==false&&EditFlag==true&&Id>0)
                    {
						repairHistory.Id = Id;
						string message = "";
						message = JIGREPAIRHISTORY_DAL.Instance.Update(repairHistory);
						if (message == "")
						{
							// Update tình trạng vào bảng thiết bị thông qua mã thiết bị.
							// Nếu thiết bị có tình trạng NG thì không được xuất nhập jig
		...
		#region Kiểm tra nhập dữ liệu của mã thiêt bị
		private bool checkControl()
		{
			...
			else
			{
				...
				if (!result[0].Equals(""))
				{
					...
					jigDvc = JIGCALDEVICES_DAL.Instance.checkCalDevicesExist(txtControlNo.Text.Trim());
					if (jigDvc != null)
					{
						richRepairReason.Focus();
					}
		...
		private void dgView_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
		{
            ...
            var eqRepair = new JIG_REPAIRHISTORY();
            eqRepair= JIGREPAIRHISTORY_DAL.Instance.getById(Id);
