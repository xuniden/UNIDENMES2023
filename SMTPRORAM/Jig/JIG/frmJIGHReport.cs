...
{
    public partial class frmJIGHReport : Form
    {
		private int InCount = 0;
		private int OutCount = 0;
		public frmJIGHReport()
        {
            InitializeComponent();
        }
        private void getInOutHistory(DateTime start, DateTime end)
        {
            ...
            dt = JIGHINOUT_DAL.Instance.SearchInOutByDate(2, "", start, end);
            if (dt.Rows.Count > 0)
            {
		...
		private void frmJIGHReport_Load(object sender, EventArgs e)
        {
            lbltotalOut.Text = "";
            lbltotalIn.Text = "";
        ...
        private void Search()
        {
			...
			else
			{
                ...
                dt=JIGHINOUT_DAL.Instance.getInOutDeviceOnline(txtSearch.Text.Trim());
				CommonDAL.Instance.ShowDataGridViewWithDataTable(dgView, dt);
                CountInOut(dt);
			}
		}    
