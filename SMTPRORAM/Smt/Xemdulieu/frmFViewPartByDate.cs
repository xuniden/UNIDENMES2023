...
{
    public partial class frmFViewPartByDate : Form
    {
        public frmFViewPartByDate()
        {
            InitializeComponent();
        }
        ...
        private void btFind_Click(object sender, EventArgs e)
        {
            ...
            if (diff.TotalDays >= 0)
            {
                try
                {
                    ...
                    dt = SmtAOIQrCodeOutputDAL.Instance.SearchPartCodeByDate(txtPartcode.Text.Trim(),DateTime.Parse(start.ToString("yyyy/MM/dd")),DateTime.Parse(endd.ToString("yyyy/MM/dd")));
                    if (dt.Rows.Count > 0)
                    {
        ...
        private void btToCsv_Click(object sender, EventArgs e)
        {
            if (dgView.Rows.Count > 0)
            {
                ...
                DateTime start = dtp_1.Value.Date;
                DateTime endd = dtp_2.Value.Date;
                dt = SmtAOIQrCodeOutputDAL.Instance.SearchPartCodeByDate(txtPartcode.Text.Trim(), start, endd);
        ...
        private void frmFViewPartByDate_Load(object sender, EventArgs e)
        {
            lblRecod.Text = "";
        }
    }
